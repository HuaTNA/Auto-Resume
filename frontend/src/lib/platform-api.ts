...
export interface AutomationJobResult { title: string; company: string; location: string; description: string; url: string; salary_min?: number; salary_max?: number; created: string; match_score: number; match_reason: string; is_new?: boolean; application_record_id?: number; approval_status?: string; materials_generated?: boolean; generation_warning?: string; }
export interface AutomationRun { id: string; automation_id: string; status: string; trigger: string; attempt_count: number; counts: Record<string, number>; result?: { query?: string; location?: string; jobs?: AutomationJobResult[]; ranking_warning?: string | null; approval_required?: boolean }; error?: string; created_at: string; }
