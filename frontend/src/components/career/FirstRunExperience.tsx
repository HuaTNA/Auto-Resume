...
export default function FirstRunExperience() {
  ...
  return (
    <>
      ...
      />
      <WorkspacePage>
        <section className="relative overflow-hidden rounded-[16px] bg-[#26332F] px-6 py-8 text-[#F8FAF8] shadow-[0_20px_60px_rgba(38,51,47,0.18)] sm:px-9 sm:py-10 lg:px-12 lg:py-12">
          <div className="relative z-10 max-w-[720px]">
            <p className="eyebrow text-[#839A90]">Resume · Evidence · Control</p>
            ...
            <p className="mt-5 max-w-xl text-[15px] leading-7 text-[#F8FAF8]">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link href={primaryHref} className="inline-flex min-h-11 items-center gap-2 rounded-[6px] bg-[#F8FAF8] px-5 text-sm font-medium text-[#26332F] shadow-sm hover:bg-[#FCFDFB]">
                <BirchIcon name={profileReady ? "bark" : "growth-ring"} size={18} />
                {primaryLabel}<span aria-hidden="true">→</span>
              </Link>
              <Link href="/search" className="inline-flex min-h-11 items-center gap-2 rounded-[6px] border border-[rgba(248,250,248,0.28)] px-5 text-sm text-[#F8FAF8] hover:bg-[rgba(248,250,248,0.08)]">
        ...
        <section className="rounded-[16px] border border-[rgba(38,51,47,0.12)] bg-[#F8FAF8] p-5 sm:p-6">
          <p className="eyebrow text-[#52645C]">Built for trust</p>
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            ...
            ].map((item) => <div key={item} className="flex items-center gap-3 text-sm text-[#26332F]"><span className="flex size-7 shrink-0 items-center justify-center rounded-[16px] bg-[#26332F] text-xs text-[#F8FAF8]">✓</span>{item}</div>)}
          </div>
          {unavailable && <p role="alert" className="mt-4 text-sm text-[#52645C]">{text("暂时无法检查职业档案状态，你仍可直接打开职业档案继续。", "We could not check profile readiness, but you can still open your career profile and continue.")}</p>}
        </section>
      </WorkspacePage>
    </>
  );
  ...
  const content = (
    <>
      <div className="flex items-center justify-between">
        <span className="flex size-10 items-center justify-center rounded-[6px] bg-[#E1EAE5]"><BirchIcon name={icon} size={22} /></span>
        <span className="latin text-xs tracking-[0.16em] text-[#52645C]">{state === "done" ? "DONE" : number}</span>
      </div>
      <h3 className="mt-5 text-lg font-medium tracking-[0.04em]">{title}</h3>
      <p className="mt-2 text-sm leading-6 text-[#52645C]">{description}</p>
    </>
  );
  const className = `block min-h-48 rounded-[16px] border p-5 ${state === "current" ? "border-[#26332F] bg-[#F8FAF8] shadow-[0_10px_30px_rgba(38,51,47,0.08)]" : "border-[rgba(38,51,47,0.12)] bg-[rgba(248,250,248,0.58)]"}`;
