...
export function Section({ title, eyebrow, action, children, className = "" }: { title: string; eyebrow?: string; action?: ReactNode; children: ReactNode; className?: string }) {
  return <section className={className}><div className="section-heading mb-3.5 flex items-end justify-between gap-4"><div>{eyebrow && <p className="latin text-xs uppercase tracking-[0.22em] text-[#7A6A50]">{eyebrow}</p>}<h2 className="mt-1 text-[1.12rem] font-medium tracking-[0.06em]">{title}</h2></div>{action}</div>{children}</section>;
}
...
export function EmptyState({ icon = "winter", title, description, action }: { icon?: BirchIconName; title: string; description: string; action?: { label: string; onClick?: () => void; href?: string } }) {
  ...
  return <div className="rounded-[12px] border border-[rgba(30,26,20,0.10)] bg-[rgba(249,245,234,0.58)] px-5 py-7 text-center"><span className="mx-auto flex size-10 items-center justify-center rounded-[6px] bg-[#EBE2CC]"><BirchIcon name={icon} size={23} /></span><h3 className="mt-3 text-sm font-medium tracking-[0.06em]">{title}</h3><p className="mx-auto mt-1.5 max-w-md text-sm leading-6 text-[#7A6A50]">{description}</p>{button}</div>;
}
