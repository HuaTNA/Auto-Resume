...
export default function InterviewPage() {
  ...
  return <><Header eyebrow={{ zh: "职业工作区", en: "CAREER WORKSPACE" }} title={{ zh: "面试准备", en: "Interview Prep" }} subtitle={{ zh: "把职位、经历故事、问题清单和复盘放进同一个语境。", en: "Keep role context, stories, questions, and debriefs together." }} action={applications.length ? <button onClick={() => setCreating(true)} className="primary-button">＋ {text("新建笔记", "New note")}</button> : undefined} /><WorkspacePage>
    ...
    <Section title={text("面试中的申请", "Applications in interview")} eyebrow={`${applications.length}`}>
      {loading ? <div className="h-28 animate-pulse rounded-[16px] bg-[rgba(38,51,47,0.05)]" /> : applications.length === 0 ? <EmptyState icon="leaf" title={text("还没有进入面试阶段的申请", "No applications in interview yet")} description={text("在申请记录中将状态改为“面试中”，这里会自动建立面试工作区。", "Move an application to Interview and it will appear here automatically.")} action={{ label: text("查看申请", "Open applications"), href: "/career/applications" }} /> : <div className="grid gap-4 sm:grid-cols-2">{applications.map((item) => <article key={item.id} className="rounded-[16px] border border-[rgba(38,51,47,0.12)] bg-[#F8FAF8] p-5"><StatusPill tone="brand">Interview</StatusPill><h3 className="mt-4 text-sm font-medium">{item.job_title}</h3><p className="mt-1 text-xs text-[#52645C]">{item.company}</p></article>)}</div>}
    </Section>
    <Section title={text("准备材料", "Preparation library")} eyebrow={`${notes.length}`}><div className="grid gap-4 md:grid-cols-2">{notes.map((note) => <article key={note.id} className="rounded-[16px] border border-[rgba(38,51,47,0.12)] bg-[#F8FAF8] p-5"><StatusPill tone={note.kind === "star" ? "brand" : "neutral"}>{note.kind}</StatusPill><h3 className="mt-4 text-sm font-medium">{note.title}</h3><p className="mt-2 whitespace-pre-wrap text-xs leading-6 text-[#52645C]">{note.content}</p></article>)}</div>{!notes.length && applications.length > 0 && <p className="text-xs text-[#52645C]">{text("还没有准备材料。", "No preparation material yet.")}</p>}</Section>
    <Link href="/knowledge?new=1" className="text-xs underline decoration-[#839A90] underline-offset-4">{text("也可以保存到知识库", "You can also save research to Knowledge")} →</Link>
  </WorkspacePage></>;
}
