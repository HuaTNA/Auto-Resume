...
export default function RegisterPage() {
  ...
  return (
    <AuthLayout mode="register">
      <p className="mb-5 text-center text-xs leading-6 text-[#52645C]">{text("从一份真实履历开始，慢慢长成你的职业知识系统。", "Begin with an honest profile and let it grow into your career knowledge system.")}</p>
      {error && <div className="mb-5 flex items-start gap-2.5 rounded-[6px] border border-[rgba(38,51,47,0.10)] bg-[#E1EAE5] px-4 py-3 text-xs text-[#26332F]"><BirchIcon name="bud" size={17} />{error}</div>}
      <form onSubmit={handleSubmit} className="space-y-4">
        {registrationMode === "closed" && <p className="rounded-[6px] bg-[#E1EAE5] px-4 py-3 text-xs">{text("目前暂停开放注册。", "Registration is currently closed.")}</p>}
...
function RegisterField({ label, icon, type, value, onChange, placeholder }: { label: string; icon: "mail" | "lock"; type: string; value: string; onChange: (value: string) => void; placeholder: string }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-[11px] tracking-[0.08em] text-[#64736C]">{label}</span>
      <span className="relative block">
        <span className="absolute left-3.5 top-1/2 -translate-y-1/2"><BirchIcon name={icon === "mail" ? "leaf" : "bark"} size={17} /></span>
        <input type={type} value={value} onChange={(event) => onChange(event.target.value)} required placeholder={placeholder} className="w-full rounded-[6px] border border-[rgba(38,51,47,0.10)] bg-[#E8EFEB] py-3 pl-11 pr-4 text-[13px] text-[#26332F] outline-none transition-colors duration-300" />
      </span>
    </label>
  );
}
