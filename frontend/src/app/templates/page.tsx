...
export default function TemplatesPage() {
  ...
  return <><Header eyebrow={{ zh: "文书 · 范式", en: "DOCUMENT FORMS · TEMPLATES" }} title={{ zh: "选择履历章法", en: "Choose a resume form" }} subtitle={{ zh: "形式应退后一步，让经历与判断成为主角。", en: "Let form recede so experience and judgment can lead." }} />
    <div className="mx-auto w-full max-w-[960px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
      <div className="mb-8 flex items-end justify-between gap-5"><div><p className="eyebrow text-[#64736C]">{text("三种克制范式", "Three measured forms")}</p><h2 className="mt-2 text-2xl font-light tracking-[0.1em]">{text("一式，一种叙述秩序", "A form for every narrative")}</h2><p className="mt-3 max-w-xl text-sm leading-7 text-[#52645C]">{text("选择与你的经历密度和目标行业最相称的表达方式。", "Choose the expression that best fits your experience and target field.")}</p></div><div className="ornament-divider hidden sm:flex" aria-hidden="true"><span /></div></div>
      {loading ? <p className="py-16 text-center text-sm text-[#52645C]">正在展开范式…</p> : error ? <p className="rounded-[6px] bg-[#E1EAE5] p-5 text-center text-sm">{error}</p> : <div className="grid gap-5 md:grid-cols-3">{templates.map((template) => { const preview = PREVIEWS[template.name] ?? PREVIEWS.classic; return <article key={template.name} onClick={() => router.push(`/generate?template=${template.name}`)} className="soft-card lift-card group cursor-pointer overflow-hidden">
        <div className="relative flex min-h-48 flex-col items-center justify-center border-b border-[rgba(38,51,47,0.12)] bg-[#E1EAE5] p-7 text-center"><span className="latin absolute left-5 top-4 text-2xl italic text-[#839A90]">{preview.index}</span>{template.name === "classic" && <span className="absolute right-4 top-4 rounded-[6px] border border-[rgba(38,51,47,0.12)] px-2 py-1 text-[9px] tracking-[0.15em] text-[#52645C]">{text("默认", "Default")}</span>}<BirchIcon name={preview.icon} size={58} /><h3 className="latin mt-5 text-xl font-normal capitalize tracking-[0.12em]">{template.name}</h3></div>
        <div className="p-6"><p className="min-h-14 text-sm leading-7 text-[#52645C]">{template.description}</p><ul className="mt-5 space-y-2.5">{preview.features.map((feature) => <li key={feature.en} className="flex items-center gap-3 text-xs text-[#52645C]"><span className="size-1 rounded-full bg-[#839A90]" />{text(feature.zh, feature.en)}</li>)}</ul><button className="primary-button mt-7 w-full">{text("采用此式", "Use this form")} <span aria-hidden="true">→</span></button></div>
      </article>; })}</div>}
    </div></>;
}
