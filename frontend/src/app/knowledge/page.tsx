...
export default function KnowledgePage() {
  ...
  return <>
    ...
    <WorkspacePage>
      ...
      <div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2"><BirchIcon name="growth-ring" size={17} /></span><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-h-12 w-full bg-[#FCFDFB] pl-11 pr-4" placeholder={text("搜索笔记、研究、链接或标签…", "Search notes, research, links, or tags…")} /></div>
      <Section title={text("知识库", "Library")} eyebrow={`${visible.length} ${text("项内容", "items")}`}>
        {visible.length === 0 ? <EmptyState icon="growth-ring" title={query ? text("没有匹配的内容", "No matching knowledge") : text("知识库还是空的", "Your library is empty")} description={query ? text("尝试更短的关键词或标签。", "Try a shorter keyword or tag.") : text("从一则笔记、一个研究结论或一条重要链接开始。", "Start with a note, a research finding, or an important link.")} action={query ? undefined : { label: text("添加第一条内容", "Add your first item"), onClick: () => setCreating(true) }} /> : <div className="grid gap-6 md:grid-cols-2">{visible.map((item) => <article key={item.id} className="lift-card rounded-[16px] border border-[rgba(38,51,47,0.12)] bg-[#F8FAF8] p-5"><div className="flex items-center justify-between"><StatusPill tone={item.kind === "research" ? "brand" : "neutral"}>{kinds[item.kind][language]}</StatusPill><div className="flex items-center gap-2"><button onClick={() => { setCreating(false); setEditing(item); }} className="text-[10px] underline decoration-[#839A90] underline-offset-4">{text("编辑", "Edit")}</button><button onClick={() => void remove(item)} className="text-[10px] text-[#52645C] underline decoration-[#839A90] underline-offset-4">{text("删除", "Delete")}</button>{item.url && <a href={item.url} target="_blank" rel="noreferrer" aria-label={text("打开来源", "Open source")} className="flex size-8 items-center justify-center rounded-[6px] hover:bg-[#FCFDFB]"><BirchIcon name="branch" size={16} /></a>}</div></div><h3 className="mt-5 text-base font-medium tracking-[0.1em]">{item.title}</h3><p className="mt-2 line-clamp-4 text-xs leading-6 text-[#52645C]">{item.content || text("暂无摘要", "No summary")}</p>{item.tags.length > 0 && <div className="mt-4 flex flex-wrap gap-1.5">{item.tags.map((tag) => <span key={tag} className="rounded-[6px] bg-[#E1EAE5] px-2 py-1 text-[9px] text-[#52645C]">#{tag}</span>)}</div>}</article>)}</div>}
      </Section>
    </WorkspacePage>
  </>;
}
