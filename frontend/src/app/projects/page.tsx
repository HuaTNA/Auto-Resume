  ...
  return <>
    ...
    <WorkspacePage>
      ...
      <Section title={text("所有项目", "All projects")} eyebrow={`${projects.filter((project) => project.status === "active").length} ${text("个正在进行", "active")}`}>
        {projects.length === 0 ? <EmptyState icon="branch" title={text("还没有项目", "No projects yet")} description={text("项目是工作站的长期语境：它可以连接任务、知识、文档、链接与自动化。", "Projects are durable workspace context, connecting tasks, knowledge, documents, links, and automations.")} action={{ label: text("创建第一个项目", "Create your first project"), onClick: () => setCreating(true) }} /> : <div className="grid gap-6 md:grid-cols-2">{projects.map((project) => <article key={project.id} className="lift-card rounded-[16px] border border-[rgba(38,51,47,0.12)] bg-[#F8FAF8] p-5 shadow-[0_2px_10px_rgba(38,51,47,0.07)]">
          <div className="flex items-start justify-between gap-4"><StatusPill tone={STATUS[project.status].tone}>{STATUS[project.status][language]}</StatusPill><div className="flex items-center gap-2"><select value={project.status} onChange={(event) => updateProject(project.id, { status: event.target.value as ProjectStatus, progress: event.target.value === "completed" ? 100 : project.progress })} aria-label={text("项目状态", "Project status")} className="min-h-8 px-2 text-[10px]">{Object.entries(STATUS).map(([value, option]) => <option key={value} value={value}>{option[language]}</option>)}</select><button onClick={() => { setCreating(false); setEditing(project); }} className="min-h-8 rounded-[6px] border border-[rgba(38,51,47,0.12)] px-2.5 text-[10px] hover:bg-[#FCFDFB]">{text("编辑", "Edit")}</button><button onClick={() => void remove(project)} className="min-h-8 px-1.5 text-[10px] text-[#52645C] underline decoration-[#839A90] underline-offset-4">{text("删除", "Delete")}</button></div></div>
          <h3 className="mt-5 text-lg font-medium tracking-[0.1em]">{project.title}</h3><p className="mt-2 min-h-12 text-xs leading-6 text-[#52645C]">{project.summary || text("尚未添加项目说明。", "No summary yet.")}</p>
          <div className="mt-5"><div className="latin mb-2 flex justify-between text-[9px] uppercase tracking-[0.24em] text-[#64736C]"><span>{text("进度", "Progress")}</span><span>{project.progress}%</span></div><input type="range" min="0" max="100" step="10" value={project.progress} onChange={(event) => updateProject(project.id, { progress: Number(event.target.value) })} className="h-1.5 w-full cursor-pointer accent-[#26332F]" /></div>
          <div className="mt-5 rounded-[6px] bg-[#E1EAE5] p-3"><p className="latin text-[9px] uppercase tracking-[0.24em] text-[#64736C]">{text("下一步", "Next action")}</p><p className="mt-1 text-xs">{project.next_action || text("还没有定义下一步", "Next action not defined")}</p></div>
          {project.due_date && <p className="mt-4 text-[10px] text-[#52645C]">{text("目标日期", "Target")} · {project.due_date}</p>}
        </article>)}</div>}
      </Section>
    </WorkspacePage>
  </>;
